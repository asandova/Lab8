...
graph.compare <- function(file1, file2, direct, plot=TRUE){
  ...
  if(direct){
    ...
    test <- all(apply(g1, 1, function(x){
      return(any(g2[,1] == x[1] & g2[,2] == x[2]) | any(g2[,2] == x[1] & g2[,1] == x[2]))
    }))
    
    test <- test & all(apply(g2, 1, function(x){
      return(any(g1[,1] == x[1] & g1[,2] == x[2]) | any(g1[,2] == x[1] & g1[,1] == x[2]))
    }))
  }
...
random.graph <- function(number.of.nodes = 100000, number.of.edges = 100000, output.file = "in_test_big.txt"){
